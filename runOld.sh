...
cmake_make_command() {
	...
	FLAGS="-DCMAKE_BUILD_TYPE=Release -DPROJECT_TARGET=${PROJECT} -DTBEasyOn=${EASY_ON} -DTBMinimalMode=${MINIMAL_MODE} -DTBFactoryMode=${FACTORY_MODE} -DCOVERITY_BUILD=${COVERITY} -DKPI_MARKERS=${KPI_MARKERS} ${TESTS_OPTION} -DCMAKE_TOOLCHAIN_FILE=../cmake/${TOOLCHAIN}"
