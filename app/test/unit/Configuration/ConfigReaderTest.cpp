...
public:
  MOCK_METHOD(std::unique_ptr<std::istream>, createInputStream,
              (const std::string &path), (const, override));
  MOCK_METHOD(std::unique_ptr<std::ostream>, createOutputStream,
              (const std::string &path), (const, override));
};
...
TEST_F(ConfigReaderTest,
       LoadSettingsFromFile_ValidSettings_CallsParseCorrectly) {
...
TEST_F(ConfigReaderTest,
       LoadSettingsFromFile_MixedValidInvalidSettings_HandlesCorrectly) {
  ...
  ASSERT_EQ(res,
            Resolution::Setting::h600w800); // Resolution::Setting::h960w1280);
                                            // - Call unit test failure
  ASSERT_EQ(dif, Difficulty::Level::Hard);
}
