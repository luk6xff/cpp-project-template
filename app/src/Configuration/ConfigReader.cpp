...
void ConfigReader::parseConfigurationFile(std::istream& inputFile,
...
void ConfigReader::processConfigurationLine(const std::string& line,
                                            Resolution::Setting& resolutionSetting,
                                            Difficulty::Level& difficultyLevel) {
    std::istringstream lineStream(line);
    std::string key;
    while (lineStream >> key) {
        if (key[0] == '#') return; // Ignore comments
...
bool ConfigReader::parseResolutionSetting(const std::string& resolutionString,
                                          Resolution::Setting& resolutionSetting) {
