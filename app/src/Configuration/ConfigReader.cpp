...
ConfigReader::ConfigReader(const std::string& configurationFilePath, const IFileStreamFactory& fileStreamFactory) :
    m_configurationFilePath(configurationFilePath), m_fileStreamFactory(fileStreamFactory)
{
}
...
void ConfigReader::parseResolution(std::istringstream& lineStream, Resolution::Setting& resolutionSetting)
{
    std::string resolutionArg;
    if (!(lineStream >> resolutionArg && parseResolutionSetting(resolutionArg, resolutionSetting)))
    {
        std::cout << "Invalid resolution setting in configuration file." << std::endl;
    }
}

void ConfigReader::parseDifficulty(std::istringstream& lineStream, Difficulty::Level& difficultyLevel)
{
...
void ConfigReader::createDefaultConfigurationFile()
{
    ...
    {
        std::cout << "Failed to create the default configuration file." << std::endl;
        return;
    }
    ...
    *outputFile << "#Resolution\n#Options: 600x800, 768x1024, 864x1152, 960x1280\n"
    ...
    if (!outputFile->good())
        std::cout << "Error occurred while writing the default configuration file." << std::endl;
}
