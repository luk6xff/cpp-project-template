 ...
 * @brief Interface for creating file streams, abstracting the file stream creation mechanism.
 *
 * This interface allows for creating input and output streams, potentially allowing
 * for different implementations depending on the underlying system or testing needs.
 */
 ...
 * @brief Concrete implementation of the IFileStreamFactory interface for file streams.
 *
 * Provides methods to create standard file input and output streams using std::ifstream and std::ofstream.
 */
class FileStreamFactory : public IFileStreamFactory {
public:
    /**
     * @brief Creates a file input stream.
     * @param path The file path to open for reading.
     ...
     * @param path The file path to open for writing. The file is truncated.
...
/**
 * @class ConfigReader
 * @brief Reads configuration settings from a file using a file stream factory.
 *
 * ConfigReader is responsible for loading game settings like resolution and difficulty
 * from a configuration file specified at the creation of the reader instance.
 */
class ConfigReader {
public:
    /**
     * @brief Constructs a ConfigReader with specified path and file stream factory.
     * @param configurationFilePath The file path to the configuration file.
     * @param fileStreamFactory Reference to an IFileStreamFactory for creating file streams.
     */
    ...
    /**
     * @brief Creates a default configuration file using provided settings.
     * @param fileStreamFactory Reference to an IFileStreamFactory for creating file streams.
     */
...
private:
    ...
    /**
     * @brief Parses the resolution setting from a string.
     * @param resolutionString The string representation of the resolution.
     * @param resolutionSetting Reference to store the parsed setting.
     * @return True if parsing was successful, false otherwise.
     */
    static bool parseResolutionSetting(const std::string& resolutionString, Resolution::Setting& resolutionSetting);

    /**
     * @brief Parses the difficulty level from a string.
     * @param difficultyString The string representation of the difficulty.
     * @param difficultyLevel Reference to store the parsed level.
     * @return True if parsing was successful, false otherwise.
     */
