...
void Game::updateMovement() {
    ...
    for (auto& enemy : m_enemies) {
        enemy.moveAIWithinBounds(m_screenWidth, (enemy.getCarType() == CarType::Truck ? m_enemyTruckSpeed : m_enemyCarSpeed) * m_delta);
    }
...
void Game::destroyObjects() {
    m_playerBullets.erase(
        std::remove_if(m_playerBullets.begin(), m_playerBullets.end(), [this](const sf::Sprite& bullet) {
    ...
    m_enemyBullets.erase(
        std::remove_if(m_enemyBullets.begin(), m_enemyBullets.end(), [this](const sf::Sprite& bullet) {
            return isOutOfScreen(bullet.getGlobalBounds());
        }),
        m_enemyBullets.end()
    );
    ...
    m_enemies.erase(
        std::remove_if(m_enemies.begin(), m_enemies.end(), [this](const GameCar& car) {
            return car.isDead();
        }),
        m_enemies.end()
    );
...
void Game::loadTextures() {
    ...
    if (!m_playerCarTexture.loadFromFile("media/player_car.png")) {
        std::cerr << "Failed to load player car texture." << std::endl;
    }
...
void Game::updateStatusTextView() {
    ...
    // Position the high score text just below the score text
...
// Initialize life indicators based on player health
...
void Game::playerMovement() {
    if (m_moveLeft) m_player->move(-m_playerSpeed * m_delta, 0);
    if (m_moveRight) m_player->move(m_playerSpeed * m_delta, 0);
    if (m_moveUp) m_player->move(0, -m_playerSpeed * m_delta);
    if (m_moveDown) m_player->move(0, m_playerSpeed * m_delta);
}
...
void Game::initPlayer() {
    m_player = std::make_unique<GameCar>(CarType::Car, GameCar::Team::Player,  (float)m_screenWidth/800, m_playerCarTexture, m_playerBulletTexture);
    m_player->setPosition((m_screenWidth / 2.0f) - m_player->getRect().width/ 2, m_screenHeight - m_player->getRect().height);
    m_player->setHealth(m_maxPlayerHealth);
}
...
void Game::spawnEnemies() {
   // Spawn as many enemies as enemyCount
    for(int i = 0; i < m_enemyCount; i++){
        ...
        if(randNum < 20) {
            m_enemies.push_back(
                GameCar{CarType::Truck, GameCar::Team::Enemy, (float)m_screenWidth/800, m_enemyTruckTextures[randNum % (m_enemyTruckTextures.size())], m_enemyBulletTexture}
            );
        }
        else {
            m_enemies.push_back(
                GameCar{CarType::Car, GameCar::Team::Enemy, (float)m_screenWidth/800, m_enemyCarTextures[randNum % (m_enemyCarTextures.size())], m_enemyBulletTexture}
            );
        }
        //Set position
