...
GameCar::GameCar(CarType type, Team team, float scale,
                 const sf::Texture &texture, const sf::Texture &bulletTexture)
    : Animation(texture), m_team(team), m_scaleMultiplier(scale) {
  m_bulletSprite.setTexture(bulletTexture);
  initializeCar(type);
}

bool GameCar::isDead() const { return m_isDestroyed; };
...
int GameCar::getDamage() const { return m_damage; }
...
std::vector<sf::Sprite> GameCar::fireBullets() {
  ...
  if (elapsed.asMilliseconds() - m_lastShotTime >= m_msBetweenShots) {
    ...
    m_bulletSprite.setPosition(carPosition.x + rect.width / 2.0f,
                               carPosition.y + bulletRect.height);
...
CarType GameCar::getCarType() const { return m_carType; }
