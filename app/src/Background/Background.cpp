...
Background::Background(const sf::Texture &bgTexture, uint32_t screenWidth,
                       uint32_t screenHeight) {
  m_width = static_cast<float>(screenWidth);
  m_height = static_cast<float>(screenHeight);

  // Factors to scale the sprite so it is the
  // same size as the screen
  sf::Vector2f factors(m_width / bgTexture.getSize().x,
                       m_height / bgTexture.getSize().y);
  ...
  // Set the second sprite just above the first
  // one to create a continuous loop effect
...
void Background::moveDown(float offset) {
  for (sf::Sprite &frame : m_frames) {
    ...
    // If the sprite moves below the screen,
    // reset its position above the screen
