...
: m_isLooping(isLooping) {
    ...
    m_playing = !m_isLooping;  // Start stopped if looping, play immediately if not
}
...
void Animation::play() {
    m_playing = true;
    m_currentFrame = 0;
    m_clock.restart();  // Restart the clock when starting the animation
}
...
class ExplosionEffect : public Animation{
    ...
    public:
        ExplosionEffect(std::vector<sf::Texture> &textures) : Animation(textures){}
