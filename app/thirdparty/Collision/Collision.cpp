...
namespace Collision
{
	...
	bool boundingBoxTest(const sf::Sprite& sprite1, const sf::Sprite& sprite2) {
		...
		auto axes = std::array<sf::Vector2f, 4>({
			...
			sf::Vector2f(OBB1.points[2].x - OBB1.points[1].x, OBB1.points[2].y - OBB1.points[1].y),
			sf::Vector2f(OBB2.points[1].x - OBB2.points[0].x, OBB2.points[1].y - OBB2.points[0].y),
			sf::Vector2f(OBB2.points[2].x - OBB2.points[1].x, OBB2.points[2].y - OBB2.points[1].y)
		});
